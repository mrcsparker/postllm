...
echo "Starting llama benchmark services..."
...
export POSTLLM_BENCH_COMPOSE_FILES="${ROOT_DIR}/compose.yaml:${ROOT_DIR}/compose.llama-e2e.yaml"
export POSTLLM_BENCH_COMPOSE_PROJECT="postllm-llama-bench"
export POSTLLM_BENCH_DOCKER_SERVICE="postgres"

SUITE_PATH="${POSTLLM_BENCH_SUITE:-${ROOT_DIR}/benchmarks/runtime_matrix.json}"
OUTPUT_DIR="${POSTLLM_BENCH_OUTPUT_DIR:-${ROOT_DIR}/target/benchmarks/llama}"

python3 "${ROOT_DIR}/scripts/benchmark_suite.py" \
  --suite "${SUITE_PATH}" \
  --output-dir "${OUTPUT_DIR}" \
  --scenario hosted-small-complete
